...
### "I published but my changes are not there"

Almost always the browser, not the deploy. A static host serves CSS and JS with
a cache lifetime (GitHub Pages sends `Cache-Control: max-age=600`) and their
filenames never change, so a visitor who has been on the site before keeps their
old stylesheet for a while after a publish. HTML edits show up, CSS and JS edits
do not — which reads exactly like "some of the changes are missing".

The build now fingerprints the stylesheets and scripts and appends `?v=<hash>`
to their URLs, so a publish that changes them changes their URLs too and the
browser has to fetch them. `main.js` passes its own version on to the page
modules it imports. The handful of modules it imports statically
(`ui`, `store`, `motion`, `cart`, `paths`) still ride the host's cache, so give
those up to ten minutes or use Ctrl+F5.

To check whether something really published, ask the server rather than the
browser:

```bash
curl -s https://chodiac.github.io/Crazy-Vinyl-Shop/assets/css/pages.css | grep "what-you-changed"
```
