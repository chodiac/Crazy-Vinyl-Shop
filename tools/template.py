...
# Fingerprint of the CSS and JS, appended to their URLs. A static host serves
# assets with a cache lifetime and their names never change, so without this a
# visitor who has been on the site before keeps their old stylesheet after a
# publish and reports the changes as missing.
ASSET_VER = ''


def set_asset_version(version):
    global ASSET_VER
    ASSET_VER = version
...
def page(*, title, description, path, body, site, categories, counts, genres,
    ...
    ver = f'?v={ASSET_VER}' if ASSET_VER else ''
    ld = ''
    if jsonld:
...
{BOOT_SCRIPT}
<link rel="stylesheet" href="/assets/css/base.css{ver}">
<link rel="stylesheet" href="/assets/css/components.css{ver}">
<link rel="stylesheet" href="/assets/css/pages.css{ver}">
{ld}
</head>
...
<script type="module" src="/assets/js/main.js{ver}"></script>
</body>
</html>
'''
